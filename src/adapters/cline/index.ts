...
/**
 * Cline Native Adapter supporting Cline CLI and VS Code extension capabilities,
 * workspace and global configuration inspection, custom modes and subagents,
 * standard MCP registration, and safe configuration lifecycle.
 */
export class ClineAdapter implements HostAdapter {
  readonly id = "cline";
  readonly name = "Cline Adapter";

  /**
   * Detects active Cline runtime environment from process environment or ancestry.
   */
  hasActiveRuntimeContext(_workspaceRoot?: string): boolean {
    if (
      process.env.CLINE_CLI === "1" ||
      process.env.CLINE_CLI === "true" ||
      process.env.CLINE_AGENT === "1" ||
      process.env.CLINE_AGENT === "true" ||
      (process.env.CLINE_SESSION && process.env.CLINE_SESSION !== "undefined") ||
      (process.env.CLINE_SESSION_ID && process.env.CLINE_SESSION_ID !== "undefined") ||
      (process.env.CLINE_PROJECT_DIR && process.env.CLINE_PROJECT_DIR !== "undefined") ||
      (process.env.CLINE_CONFIG_DIR && process.env.CLINE_CONFIG_DIR !== "undefined") ||
      (process.env.CLINE_VERSION && process.env.CLINE_VERSION !== "undefined") ||
      (process.env.CLINE_MODEL && process.env.CLINE_MODEL !== "undefined")
    ) {
      return true;
    }
    if (process.env._ && path.basename(process.env._).toLowerCase().includes("cline")) {
      return true;
    }
    if (process.title && path.basename(process.title).toLowerCase().includes("cline")) {
      return true;
    }
    return false;
  }
  ...
  /**
   * Identifies if Cline is the host harness for this workspace.
   */
  async identifyHost(workspaceRoot?: string): Promise<boolean> {
    ...
    if (workspaceRoot) {
      const candidates = [
        ...
        path.join(workspaceRoot, ".cline", "settings.json"),
        path.join(workspaceRoot, ".cline", "mcp.json"),
        path.join(workspaceRoot, ".cline", "cline_mcp_settings.json"),
        path.join(workspaceRoot, "cline.json"),
      ];
    ...
    if (process.env.CLINE_CONFIG_DIR && fs.existsSync(process.env.CLINE_CONFIG_DIR)) {
      return true;
    }
    ...
    if (!workspaceRoot) {
      const userCandidates = [
        ...
        path.join(os.homedir(), ".cline", "settings.json"),
        path.join(
          os.homedir(),
          ".config",
          "Code",
  ...
  /**
   * Inspects Cline host version and determines compatibility.
   */
  async inspectVersion(workspaceRoot?: string): Promise<HostVersionInfo> {
    let version = process.env.CLINE_VERSION || process.env.CLINE_CLI_VERSION;
    let raw: string | undefined = version;

    if (!version && workspaceRoot) {
      const versionFile = path.join(workspaceRoot, ".cline", "version");
      if (fs.existsSync(versionFile)) {
        ...
        const configFile = path.join(workspaceRoot, ".cline", "settings.json");
    ...
    if (!version) {
      const cliResult = await this.runCliCommand("cline", ["--version"], workspaceRoot);
    ...
    if (!version) {
      const userVersionFile = path.join(os.homedir(), ".cline", "version");
  ...
  determineTargetConfigPath(workspaceRoot: string): string {
    ...
    const settingsFile = path.join(clineDir, "settings.json");
    if (fs.existsSync(settingsFile)) return settingsFile;

    const rootRules = path.join(workspaceRoot, ".clinerules");
    if (fs.existsSync(rootRules)) return rootRules;
  ...
  determineMcpRegistrationPath(workspaceRoot: string): string {
    const mcpFile = path.join(workspaceRoot, ".cline", "mcp.json");
    if (fs.existsSync(mcpFile)) return mcpFile;

    const clineMcpFile = path.join(workspaceRoot, ".cline", "cline_mcp_settings.json");
    if (fs.existsSync(clineMcpFile)) return clineMcpFile;

    const settingsFile = path.join(workspaceRoot, ".cline", "settings.json");
  ...
  async inspectCapabilities(workspaceRoot?: string): Promise<HostCapabilities> {
    ...
    if (process.env.CLINE_MAX_CONCURRENCY) {
      const parsed = parseInt(process.env.CLINE_MAX_CONCURRENCY, 10);
    ...
    if (workspaceRoot) {
      const agentsDir = path.join(workspaceRoot, ".cline", "agents");
      const rulesFile = path.join(workspaceRoot, ".clinerules");
      const modesFile = path.join(workspaceRoot, ".roomodes");
      if (fs.existsSync(agentsDir)) {
        subagentsState = "available";
        subagentsEvidenceLocator = ".cline/agents";
      } else if (fs.existsSync(rulesFile)) {
        subagentsState = "available";
        subagentsEvidenceLocator = ".clinerules";
    ...
    if (subagentsState === "unknown" && config) {
      if (
        ...
        subagentsState = "available";
        subagentsEvidenceLocator = "cline.config.customModes";
      }
    }
    ...
    if (workspaceRoot) {
      const threadsDir = path.join(workspaceRoot, ".cline", "threads");
      if (fs.existsSync(threadsDir)) {
        threadsState = "available";
        threadsEvidenceLocator = ".cline/threads";
      }
    }
    ...
    if (threadsState === "unknown" && config) {
      if (config.threads === true) {
        threadsState = "available";
        threadsEvidenceLocator = "cline.config.threads";
      }
    }
  ...
  /**
   * Inspects models evidenced in Cline configurations or environment.
   */
  async inspectModels(workspaceRoot?: string): Promise<HostModel[]> {
    ...
    if (process.env.CLINE_MODEL) {
      addModel(process.env.CLINE_MODEL, process.env.CLINE_MODEL, "process.env.CLINE_MODEL");
    }
  ...
  async inspectEffortValues(workspaceRoot?: string): Promise<string[]> {
    ...
    if (process.env.CLINE_REASONING_EFFORT) {
      return [process.env.CLINE_REASONING_EFFORT];
    }
  ...
  /**
   * Inspects native reasoning options for Cline.
   */
  ...
  /**
   * Resolves abstract reasoning policy to Cline native representation.
   */
  ...
  /**
   * Inspects execution topology capabilities for Cline.
   */
  ...
  async inspectCompanionRegistration(
    ...
    const workspaceMcpCandidates = [
      path.join(workspace, ".cline", "mcp.json"),
      path.join(workspace, ".cline", "cline_mcp_settings.json"),
      path.join(workspace, ".cline", "settings.json"),
    ];
  ...
  /**
   * Previews companion registration into Cline configuration.
   */
  async previewCompanionRegistration(
    ...
    if (fs.existsSync(targetFile)) {
      ...
      if (parseErrors.length > 0) {
        return {
          ...
          target_file: targetFile,
          mutation_targets: [],
          error: `Cline configuration file at '${targetFile}' contains syntax errors. Rejecting companion registration to prevent loss.`,
        };
      }
    ...
    const previewId = `preview-companion-cline-${Date.now()}`;
  ...
  async applyCompanionRegistration(
    ...
    if (!preview.supported || !preview.files || preview.files.length === 0) {
      return {
        ...
        applied_targets: [],
        error: preview.error || "Cannot apply companion registration for Cline.",
      };
    }
    ...
    return {
      ...
      message: `Cline companion MCP registration applied successfully to ${appliedTargets.join(", ")}.`,
    };
  }
  ...
  async validateCompanionRegistration(workspaceRoot?: string): Promise<ValidationResult> {
    ...
    return {
      ...
      message: status.registered
        ? `Cline companion registered at ${status.target_file}`
        : "Cline companion is not registered",
      details: status,
    };
  }
  ...
  async renderConfiguration(
    ...
    if (fs.existsSync(targetFile)) {
      ...
      if (parseErrors.length > 0) {
        throw new Error(
          `Cline configuration file at '${targetFile}' contains syntax errors. Rejecting mutation to prevent configuration loss.`
        );
      }
      initialText = existingContent;
    }
    ...
    // 1. Update model (model and apiConfiguration.apiModelId)
    ...
    const previewId = `preview-cline-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
  ...
  async applyConfiguration(
    ...
    return {
      ...
      preview_id: previewId,
      applied_targets: appliedTargets,
      message: `Cline configuration applied successfully to ${appliedTargets.length} target(s).`,
    };
  }
  ...
  async validateConfiguration(
    ...
    if (!fs.existsSync(targetFile)) {
      return {
        valid: false,
        workspace,
        message: `Cline configuration file '${targetFile}' does not exist.`,
    ...
    try {
      ...
      return {
        valid: false,
        workspace,
        message: `Cline configuration file '${targetFile}' is invalid: ${e.message}`,
        errors: [e.message],
      };
    }
    ...
    return {
      ...
      message:
        errors.length === 0
          ? "Cline configuration matches expected plan."
          : `Cline configuration validation failed: ${errors.join("; ")}`,
