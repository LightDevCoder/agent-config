...
/**
 * Roo Code Native Adapter supporting Roo Code CLI and extension capabilities,
 * custom modes, .roomodes definitions, workspace and global configuration inspection,
 * standard MCP registration, and safe configuration lifecycle.
 */
export class RooCodeAdapter implements HostAdapter {
  readonly id = "roo-code";
  readonly name = "Roo Code Adapter";

  /**
   * Detects active Roo Code runtime environment from process environment or ancestry.
   */
  hasActiveRuntimeContext(_workspaceRoot?: string): boolean {
    if (
      process.env.ROO_CODE_CLI === "1" ||
      process.env.ROO_CODE_CLI === "true" ||
      process.env.ROO_AGENT === "1" ||
      process.env.ROO_AGENT === "true" ||
      (process.env.ROO_SESSION && process.env.ROO_SESSION !== "undefined") ||
      (process.env.ROO_SESSION_ID && process.env.ROO_SESSION_ID !== "undefined") ||
      (process.env.ROO_PROJECT_DIR && process.env.ROO_PROJECT_DIR !== "undefined") ||
      (process.env.ROO_CONFIG_DIR && process.env.ROO_CONFIG_DIR !== "undefined") ||
      (process.env.ROO_VERSION && process.env.ROO_VERSION !== "undefined") ||
      (process.env.ROO_CODE_VERSION && process.env.ROO_CODE_VERSION !== "undefined") ||
      (process.env.ROO_MODEL && process.env.ROO_MODEL !== "undefined")
    ) {
      return true;
    }
    if (
      process.env._ &&
      (path.basename(process.env._).toLowerCase().includes("roo-code") ||
        path.basename(process.env._).toLowerCase().includes("roocode"))
    ) {
      return true;
    }
    if (
      process.title &&
      (path.basename(process.title).toLowerCase().includes("roo-code") ||
        path.basename(process.title).toLowerCase().includes("roocode"))
    ) {
      return true;
    }
    return false;
  ...
  /**
   * Identifies if Roo Code is the host harness for this workspace.
   */
  async identifyHost(workspaceRoot?: string): Promise<boolean> {
    ...
    if (workspaceRoot) {
      const candidates = [
        path.join(workspaceRoot, ".roo"),
        path.join(workspaceRoot, ".roomodes"),
        path.join(workspaceRoot, ".roocode"),
        path.join(workspaceRoot, ".roo", "settings.json"),
        path.join(workspaceRoot, ".roo", "mcp.json"),
        path.join(workspaceRoot, ".roo", "roo_mcp_settings.json"),
        path.join(workspaceRoot, ".roocode", "mcp.json"),
        path.join(workspaceRoot, "roo.json"),
      ];
    ...
    if (process.env.ROO_CONFIG_DIR && fs.existsSync(process.env.ROO_CONFIG_DIR)) {
      return true;
    }
    ...
    if (!workspaceRoot) {
      const userCandidates = [
        ...
        path.join(os.homedir(), ".roo", "settings.json"),
        path.join(
          os.homedir(),
          ".config",
          "Code",
  ...
  /**
   * Inspects Roo Code host version and determines compatibility.
   */
  async inspectVersion(workspaceRoot?: string): Promise<HostVersionInfo> {
    let version =
      process.env.ROO_VERSION ||
      process.env.ROO_CODE_VERSION ||
      process.env.ROO_CLI_VERSION;
    let raw: string | undefined = version;

    if (!version && workspaceRoot) {
      const versionFile = path.join(workspaceRoot, ".roo", "version");
      if (fs.existsSync(versionFile)) {
        ...
        const configFile = path.join(workspaceRoot, ".roo", "settings.json");
    ...
    if (!version) {
      const cliResult = await this.runCliCommand("roo-code", ["--version"], workspaceRoot);
    ...
    if (!version) {
      const userVersionFile = path.join(os.homedir(), ".roo", "version");
  ...
  determineTargetConfigPath(workspaceRoot: string): string {
    ...
    const settingsFile = path.join(rooDir, "settings.json");
    if (fs.existsSync(settingsFile)) return settingsFile;

    const rootModes = path.join(workspaceRoot, ".roomodes");
    if (fs.existsSync(rootModes)) return rootModes;
  ...
  determineMcpRegistrationPath(workspaceRoot: string): string {
    const mcpFile = path.join(workspaceRoot, ".roo", "mcp.json");
    if (fs.existsSync(mcpFile)) return mcpFile;

    const rooMcpFile = path.join(workspaceRoot, ".roo", "roo_mcp_settings.json");
    if (fs.existsSync(rooMcpFile)) return rooMcpFile;

    const settingsFile = path.join(workspaceRoot, ".roo", "settings.json");
  ...
  async inspectCapabilities(workspaceRoot?: string): Promise<HostCapabilities> {
    ...
    if (process.env.ROO_MAX_CONCURRENCY) {
      const parsed = parseInt(process.env.ROO_MAX_CONCURRENCY, 10);
    ...
    // Subagent / custom modes detection
    let subagentsState: "available" | "unknown" = "unknown";
    let subagentsEvidenceLocator: string | undefined;

    if (workspaceRoot) {
      const agentsDir = path.join(workspaceRoot, ".roo", "agents");
      const modesFile = path.join(workspaceRoot, ".roomodes");
      if (fs.existsSync(agentsDir)) {
        subagentsState = "available";
        subagentsEvidenceLocator = ".roo/agents";
      } else if (fs.existsSync(modesFile)) {
        subagentsState = "available";
        subagentsEvidenceLocator = ".roomodes";
      }
    }
    ...
    if (subagentsState === "unknown" && config) {
      if (
        ...
        Array.isArray(config.customModes) ||
        Array.isArray(config.modes) ||
        (config.agents && typeof config.agents === "object")
      ) {
        subagentsState = "available";
        subagentsEvidenceLocator = "roo.config.customModes";
      }
    }
    ...
    if (workspaceRoot) {
      const threadsDir = path.join(workspaceRoot, ".roo", "threads");
      if (fs.existsSync(threadsDir)) {
        threadsState = "available";
        threadsEvidenceLocator = ".roo/threads";
      }
    }
    ...
    if (threadsState === "unknown" && config) {
      if (config.threads === true) {
        threadsState = "available";
        threadsEvidenceLocator = "roo.config.threads";
      }
    }
        ...
        reasoning: {
          ...
          ...(reasoningAvailable
            ? {
                evidence: {
                  kind: "host-config",
                  locator: "apiConfiguration.thinkingBudget",
                },
              }
            : {}),
        },
  ...
  /**
   * Inspects models evidenced in Roo Code configurations or environment.
   */
  async inspectModels(workspaceRoot?: string): Promise<HostModel[]> {
    ...
    if (workspaceRoot) {
        ...
        if (fs.existsSync(cp)) {
          try {
            ...
            if (parsed) {
              ...
              if (typeof parsed.modelId === "string") addModel(parsed.modelId, parsed.modelId, cp);
              if (typeof parsed.apiConfiguration?.apiModelId === "string") {
                addModel(
                  parsed.apiConfiguration.apiModelId,
                  parsed.apiConfiguration.apiModelId,
                  cp
                );
              }
      ...
      // Modes in .roomodes
      const roomodesFile = path.join(workspaceRoot, ".roomodes");
      if (fs.existsSync(roomodesFile)) {
        try {
          const parsed = jsonc.parse(fs.readFileSync(roomodesFile, "utf-8"));
          if (parsed && Array.isArray(parsed.customModes)) {
            for (const mode of parsed.customModes) {
              if (mode && typeof mode.model === "string") {
                addModel(mode.model, mode.name || mode.model, roomodesFile);
              }
            }
          }
        } catch {
    ...
    if (process.env.ROO_MODEL) {
      addModel(process.env.ROO_MODEL, process.env.ROO_MODEL, "process.env.ROO_MODEL");
    }
  ...
  async inspectEffortValues(workspaceRoot?: string): Promise<string[]> {
    ...
    if (config) {
      ...
      if (typeof config.apiConfiguration?.thinkingBudget === "string") {
        return [config.apiConfiguration.thinkingBudget];
      }
    }

    if (process.env.ROO_REASONING_EFFORT) {
      return [process.env.ROO_REASONING_EFFORT];
    }
  ...
  /**
   * Inspects native reasoning options for Roo Code.
   */
  async inspectReasoningOptions(workspaceRoot?: string): Promise<HostReasoningOptions> {
    const effortValues = await this.inspectEffortValues(workspaceRoot);
    return {
      native_field: "apiConfiguration.thinkingBudget",
  ...
  /**
   * Resolves abstract reasoning policy to Roo Code native representation.
   */
  ...
  /**
   * Inspects execution topology capabilities for Roo Code.
   */
  ...
  async inspectCompanionRegistration(
    ...
    const workspaceMcpCandidates = [
      path.join(workspace, ".roo", "mcp.json"),
      path.join(workspace, ".roo", "roo_mcp_settings.json"),
      path.join(workspace, ".roocode", "mcp.json"),
      path.join(workspace, ".roo", "settings.json"),
    ];
    ...
    const userMcpCandidates = [
      path.join(os.homedir(), ".roo", "mcp.json"),
      path.join(os.homedir(), ".roo", "roo_mcp_settings.json"),
      path.join(os.homedir(), ".roo", "settings.json"),
      path.join(
        os.homedir(),
        ".config",
  ...
  /**
   * Previews companion registration into Roo Code configuration.
   */
  async previewCompanionRegistration(
    ...
    if (fs.existsSync(targetFile)) {
      ...
      if (parseErrors.length > 0) {
        return {
          ...
          target_file: targetFile,
          mutation_targets: [],
          error: `Roo Code configuration file at '${targetFile}' contains syntax errors. Rejecting companion registration to prevent loss.`,
        };
      }
    ...
    const previewId = `preview-companion-roo-code-${Date.now()}`;
  ...
  async applyCompanionRegistration(
    ...
    if (!preview.supported || !preview.files || preview.files.length === 0) {
      return {
        ...
        applied_targets: [],
        error: preview.error || "Cannot apply companion registration for Roo Code.",
      };
    }
    ...
    return {
      ...
      message: `Roo Code companion MCP registration applied successfully to ${appliedTargets.join(", ")}.`,
    };
  }
  ...
  async validateCompanionRegistration(workspaceRoot?: string): Promise<ValidationResult> {
    ...
    return {
      ...
      message: status.registered
        ? `Roo Code companion registered at ${status.target_file}`
        : "Roo Code companion is not registered",
      details: status,
    };
  }
  ...
  async renderConfiguration(
    ...
    if (fs.existsSync(targetFile)) {
      ...
      if (parseErrors.length > 0) {
        throw new Error(
          `Roo Code configuration file at '${targetFile}' contains syntax errors. Rejecting mutation to prevent configuration loss.`
        );
      }
      initialText = existingContent;
    }
    ...
    const previewId = `preview-roo-code-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
  ...
  async applyConfiguration(
    ...
    return {
      ...
      preview_id: previewId,
      applied_targets: appliedTargets,
      message: `Roo Code configuration applied successfully to ${appliedTargets.length} target(s).`,
    };
  }
  ...
  async validateConfiguration(
    ...
    if (!fs.existsSync(targetFile)) {
      return {
        valid: false,
        workspace,
        message: `Roo Code configuration file '${targetFile}' does not exist.`,
    ...
    try {
      ...
      return {
        valid: false,
        workspace,
        message: `Roo Code configuration file '${targetFile}' is invalid: ${e.message}`,
        errors: [e.message],
      };
    }
    ...
    const actualModel =
      parsed?.model || parsed?.modelId || parsed?.apiConfiguration?.apiModelId;
    ...
    return {
      ...
      message:
        errors.length === 0
          ? "Roo Code configuration matches expected plan."
          : `Roo Code configuration validation failed: ${errors.join("; ")}`,
