...
/**
 * Windsurf / Cascade Native Adapter supporting Windsurf IDE, Cascade agent,
 * workspace and global Codeium/Windsurf configurations, standard MCP registration,
 * model inventory, and safe configuration lifecycle.
 */
export class WindsurfAdapter implements HostAdapter {
  readonly id = "windsurf";
  readonly name = "Windsurf Adapter";

  /**
   * Detects active Windsurf / Cascade runtime environment from process environment or ancestry.
   */
  hasActiveRuntimeContext(_workspaceRoot?: string): boolean {
    if (
      process.env.WINDSURF_CLI === "1" ||
      process.env.WINDSURF_CLI === "true" ||
      process.env.WINDSURF_AGENT === "1" ||
      process.env.WINDSURF_AGENT === "true" ||
      process.env.CASCADE_AGENT === "1" ||
      process.env.CASCADE_AGENT === "true" ||
      (process.env.WINDSURF_SESSION && process.env.WINDSURF_SESSION !== "undefined") ||
      (process.env.WINDSURF_SESSION_ID && process.env.WINDSURF_SESSION_ID !== "undefined") ||
      (process.env.CASCADE_SESSION_ID && process.env.CASCADE_SESSION_ID !== "undefined") ||
      (process.env.WINDSURF_PROJECT_DIR && process.env.WINDSURF_PROJECT_DIR !== "undefined") ||
      (process.env.WINDSURF_CONFIG_DIR && process.env.WINDSURF_CONFIG_DIR !== "undefined") ||
      (process.env.WINDSURF_VERSION && process.env.WINDSURF_VERSION !== "undefined") ||
      (process.env.CASCADE_VERSION && process.env.CASCADE_VERSION !== "undefined") ||
      (process.env.WINDSURF_MODEL && process.env.WINDSURF_MODEL !== "undefined") ||
      (process.env.CASCADE_MODEL && process.env.CASCADE_MODEL !== "undefined")
    ) {
      return true;
    }
    if (
      process.env._ &&
      (path.basename(process.env._).toLowerCase().includes("windsurf") ||
        path.basename(process.env._).toLowerCase().includes("cascade"))
    ) {
      return true;
    }
    if (
      process.title &&
      (path.basename(process.title).toLowerCase().includes("windsurf") ||
        path.basename(process.title).toLowerCase().includes("cascade"))
    ) {
      return true;
    }
    return false;
  ...
  /**
   * Identifies if Windsurf is the host harness for this workspace.
   */
  async identifyHost(workspaceRoot?: string): Promise<boolean> {
    ...
    if (workspaceRoot) {
      const candidates = [
        ...
        path.join(workspaceRoot, ".codeium", "windsurf"),
        path.join(workspaceRoot, ".windsurfrules"),
        path.join(workspaceRoot, ".windsurf", "settings.json"),
        path.join(workspaceRoot, ".windsurf", "mcp.json"),
        path.join(workspaceRoot, ".codeium", "windsurf", "mcp_config.json"),
        path.join(workspaceRoot, ".codeium", "windsurf", "settings.json"),
        path.join(workspaceRoot, "windsurf.json"),
      ];
    ...
    if (process.env.WINDSURF_CONFIG_DIR && fs.existsSync(process.env.WINDSURF_CONFIG_DIR)) {
      return true;
    }
    ...
    if (!workspaceRoot) {
      const userCandidates = [
        ...
        path.join(os.homedir(), ".windsurf", "settings.json"),
      ];
  ...
  /**
   * Inspects Windsurf host version and determines compatibility.
   */
  async inspectVersion(workspaceRoot?: string): Promise<HostVersionInfo> {
    let version =
      process.env.WINDSURF_VERSION ||
      process.env.CASCADE_VERSION ||
      process.env.WINDSURF_CLI_VERSION;
    let raw: string | undefined = version;

    if (!version && workspaceRoot) {
      const versionFileCandidates = [
        path.join(workspaceRoot, ".windsurf", "version"),
        path.join(workspaceRoot, ".codeium", "windsurf", "version"),
      ];
      for (const vf of versionFileCandidates) {
        if (fs.existsSync(vf)) {
          try {
            raw = fs.readFileSync(vf, "utf-8").trim();
            version = raw;
            break;
          } catch {
            // Skip
    ...
    if (!version) {
      const cliResult = await this.runCliCommand("windsurf", ["--version"], workspaceRoot);
    ...
    if (!version) {
      const userVersionCandidates = [
        path.join(os.homedir(), ".windsurf", "version"),
        path.join(os.homedir(), ".codeium", "windsurf", "version"),
      ];
      for (const vf of userVersionCandidates) {
        if (fs.existsSync(vf)) {
          try {
            raw = fs.readFileSync(vf, "utf-8").trim();
            version = raw;
  ...
  determineTargetConfigPath(workspaceRoot: string): string {
    ...
    const codeiumSettings = path.join(workspaceRoot, ".codeium", "windsurf", "settings.json");
    if (fs.existsSync(codeiumSettings)) return codeiumSettings;
  ...
  determineMcpRegistrationPath(workspaceRoot: string): string {
    const codeiumMcp = path.join(workspaceRoot, ".codeium", "windsurf", "mcp_config.json");
    if (fs.existsSync(codeiumMcp)) return codeiumMcp;

    const windsurfMcp = path.join(workspaceRoot, ".windsurf", "mcp.json");
    if (fs.existsSync(windsurfMcp)) return windsurfMcp;

    const windsurfSettings = path.join(workspaceRoot, ".windsurf", "settings.json");
    if (fs.existsSync(windsurfSettings)) {
      try {
        const parsed = jsonc.parse(fs.readFileSync(windsurfSettings, "utf-8"));
        if (parsed?.mcpServers) return windsurfSettings;
      } catch {
        // Skip
      }
    }
  ...
  async inspectCapabilities(workspaceRoot?: string): Promise<HostCapabilities> {
    ...
    if (process.env.WINDSURF_MAX_CONCURRENCY) {
      const parsed = parseInt(process.env.WINDSURF_MAX_CONCURRENCY, 10);
    ...
    if (workspaceRoot) {
      const agentsDir = path.join(workspaceRoot, ".windsurf", "agents");
      if (fs.existsSync(agentsDir)) {
        subagentsState = "available";
        subagentsEvidenceLocator = ".windsurf/agents";
      }
    }
    ...
    if (subagentsState === "unknown" && config) {
      if (
        ...
        config["cascade.subagents"] === true ||
        (config.agents && typeof config.agents === "object")
      ) {
        subagentsState = "available";
        subagentsEvidenceLocator = "windsurf.config.agents";
      }
    }
    ...
    if (workspaceRoot) {
      const threadsDir = path.join(workspaceRoot, ".windsurf", "threads");
      if (fs.existsSync(threadsDir)) {
        threadsState = "available";
        threadsEvidenceLocator = ".windsurf/threads";
      }
    }

    if (threadsState === "unknown" && config) {
      if (config.threads === true || config["cascade.threads"] === true) {
        threadsState = "available";
        threadsEvidenceLocator = "windsurf.config.threads";
      }
    }
        ...
        reasoning: {
          ...
          ...(reasoningAvailable
            ? {
                evidence: {
                  kind: "host-config",
                  locator: "cascade.reasoningEffort",
                },
              }
            : {}),
        },
  ...
  /**
   * Inspects models evidenced in Windsurf / Cascade configurations or environment.
   */
  async inspectModels(workspaceRoot?: string): Promise<HostModel[]> {
    ...
    const addModel = (id: string, label?: string, locator?: string) => {
      ...
      models.push({
        ...
        evidence: {
          kind: "host-config",
          locator: locator || "windsurf-config",
        },
      });
    };
        ...
        if (fs.existsSync(cp)) {
          try {
            ...
            if (parsed) {
              ...
              if (typeof parsed["cascade.model"] === "string")
                addModel(parsed["cascade.model"], parsed["cascade.model"], cp);
              if (typeof parsed["windsurf.model"] === "string")
                addModel(parsed["windsurf.model"], parsed["windsurf.model"], cp);
    ...
    const userConfigs = [
      path.join(os.homedir(), ".codeium", "windsurf", "settings.json"),
      path.join(os.homedir(), ".windsurf", "settings.json"),
      path.join(os.homedir(), ".config", "Windsurf", "User", "settings.json"),
    ];
    ...
    if (process.env.WINDSURF_MODEL) {
      addModel(process.env.WINDSURF_MODEL, process.env.WINDSURF_MODEL, "process.env.WINDSURF_MODEL");
    }
    if (process.env.CASCADE_MODEL) {
      addModel(process.env.CASCADE_MODEL, process.env.CASCADE_MODEL, "process.env.CASCADE_MODEL");
    }
  ...
  async inspectEffortValues(workspaceRoot?: string): Promise<string[]> {
    ...
    if (config) {
      ...
      if (typeof config["cascade.reasoningEffort"] === "string") {
        return [config["cascade.reasoningEffort"]];
      }
      if (typeof config["windsurf.reasoningEffort"] === "string") {
        return [config["windsurf.reasoningEffort"]];
      }
    ...
    if (process.env.WINDSURF_REASONING_EFFORT) {
      return [process.env.WINDSURF_REASONING_EFFORT];
    }
    if (process.env.CASCADE_REASONING_EFFORT) {
      return [process.env.CASCADE_REASONING_EFFORT];
    }
  ...
  /**
   * Inspects native reasoning options for Windsurf.
   */
  async inspectReasoningOptions(workspaceRoot?: string): Promise<HostReasoningOptions> {
    const effortValues = await this.inspectEffortValues(workspaceRoot);
    return {
      native_field: "cascade.reasoningEffort",
  ...
  /**
   * Resolves abstract reasoning policy to Windsurf native representation.
   */
  ...
  /**
   * Inspects execution topology capabilities for Windsurf.
   */
  ...
  async inspectCompanionRegistration(
    ...
    const workspaceMcpCandidates = [
      path.join(workspace, ".codeium", "windsurf", "mcp_config.json"),
      path.join(workspace, ".windsurf", "mcp.json"),
      path.join(workspace, ".windsurf", "settings.json"),
    ];
  ...
  /**
   * Previews companion registration into Windsurf configuration.
   */
  async previewCompanionRegistration(
    ...
    if (fs.existsSync(targetFile)) {
      ...
      if (parseErrors.length > 0) {
        return {
          ...
          target_file: targetFile,
          mutation_targets: [],
          error: `Windsurf configuration file at '${targetFile}' contains syntax errors. Rejecting companion registration to prevent loss.`,
        };
      }
    ...
    const previewId = `preview-companion-windsurf-${Date.now()}`;
  ...
  async applyCompanionRegistration(
    ...
    if (!preview.supported || !preview.files || preview.files.length === 0) {
      return {
        ...
        applied_targets: [],
        error: preview.error || "Cannot apply companion registration for Windsurf.",
      };
    }
    ...
    return {
      ...
      message: `Windsurf companion MCP registration applied successfully to ${appliedTargets.join(", ")}.`,
    };
  }
  ...
  async validateCompanionRegistration(workspaceRoot?: string): Promise<ValidationResult> {
    ...
    return {
      ...
      message: status.registered
        ? `Windsurf companion registered at ${status.target_file}`
        : "Windsurf companion is not registered",
      details: status,
    };
  }
  ...
  async renderConfiguration(
    ...
    if (fs.existsSync(targetFile)) {
      ...
      if (parseErrors.length > 0) {
        throw new Error(
          `Windsurf configuration file at '${targetFile}' contains syntax errors. Rejecting mutation to prevent configuration loss.`
        );
      }
      initialText = existingContent;
    }
    ...
    // 1. Update model
    const modelEdits = jsonc.modify(currentText, ["cascade.model"], targetModel, formatting);
    ...
    const previewId = `preview-windsurf-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
  ...
  async applyConfiguration(
    ...
    return {
      ...
      preview_id: previewId,
      applied_targets: appliedTargets,
      message: `Windsurf configuration applied successfully to ${appliedTargets.length} target(s).`,
    };
  }
  ...
  async validateConfiguration(
    ...
    if (!fs.existsSync(targetFile)) {
      return {
        valid: false,
        workspace,
        message: `Windsurf configuration file '${targetFile}' does not exist.`,
    ...
    try {
      ...
      return {
        valid: false,
        workspace,
        message: `Windsurf configuration file '${targetFile}' is invalid: ${e.message}`,
        errors: [e.message],
      };
    }
    ...
    const actualModel = parsed?.["cascade.model"] || parsed?.model || parsed?.["windsurf.model"];
    ...
    return {
      ...
      message:
        errors.length === 0
          ? "Windsurf configuration matches expected plan."
          : `Windsurf configuration validation failed: ${errors.join("; ")}`,
